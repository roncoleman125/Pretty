 ...
 *
 * Internal routines:
 *      make0() - bind and scan everything to make a TAR
 *      make0sort() - reorder TARS chain by their time (newest to oldest)
 *
 ...
 * 04/11/94 (seiwald) - Combined deps & headers into deps[2] in TAR.
 ...
 * 01/14/03 (seiwald) - fix includes fix with new internal includes TAR
...
static void make0( TAR *t, TARGET *p, int depth, 
                COUNTS *counts, int anyhow );

static TARS *make0sort( TARGETS *c );
