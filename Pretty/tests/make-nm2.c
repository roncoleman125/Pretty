 ...
 *
 * Internal routines:
 *      make0() - bind and scan everything to make a TA
 *      make0sort() - reorder TAS chain by their time (newest to oldest)
 *
 ...
 * 04/11/94 (seiwald) - Combined deps & headers into deps[2] in TA.
 ...
 * 01/03/03 (seiwald) - T_NEWER once again gets set with missing parent
 * 01/14/03 (seiwald) - fix includes fix with new internal includes TA
 * 04/04/03 (seiwald) - fix INTERNAL node b1 to avoid T_BIND_PARENTS
 */
...
static void make0( TA *t, TA *p, int depth, 
                COUNTS *counts, int anyhow );

static TAS *make0sort( TAS *c );
        ...
        int     anyhow )        /* forcibly touch all (real) targets */
{
        ...
        if( t->b1 == T_BIND_UNBOUND && !( t->flags & T_FLAG_NOTFILE ) )
        {
            t->b2 = search( t->name, &t->time );
            t->b1 = t->time ? T_BIND_EXISTS : T_BIND_MISSING;
        }
        ...
        if( DEBUG_MAKEPROG )
        {
            if( strcmp( t->name, t->b2 ) )
            {
                printf( "bind\t--\t%s%s: %s\n",
                        spaces( depth ), t->name, t->b2 );
            }
            ...
            case T_BIND_PARENTS:
                printf( "time\t--\t%s%s: %s\n",
                        spaces( depth ), t->name, target_bind[ t->b1 ] );
                break;
        ...
        for( c = t->depends; c; c = c->next )
        {
            ...
            if( c->target->fate == T_INIT )
                make0( c->target, ptime, depth + 1, counts, anyhow );
            else if( c->target->fate == T_MAKING && !internal )
         ...
         * based only upon other flags and our b1 (done later).
         */
        ...
        */
        ...
        else if( fate >= T_SPOIL )
        {
            fate = T_UPDATE;
        }
        else if( t->b1 == T_BIND_MISSING )
        {
            fate = T_MISSING;
        }
        else if( t->b1 == T_BIND_EXISTS && last > t->time )
        {
            fate = T_OUTDATED;
        }
        else if( t->b1 == T_BIND_PARENTS && last > p->time )
        {
            fate = T_NEEDTMP;
        }
        else if( t->b1 == T_BIND_PARENTS && hlast > p->time )
        {
            fate = T_NEEDTMP;
        }
        ...
        else if( t->b1 == T_BIND_EXISTS && t->flags & T_FLAG_TEMP )
        {
            fate = T_ISTMP;
        }
        else if( t->b1 == T_BIND_EXISTS && p && 
                 p->b1 != T_BIND_UNBOUND && t->time > p->time )
        {
            fate = T_NEWER;
        }
        else
        ...
        if( fate == T_MISSING && !t->actions && !t->depends )
        {
        ...
        if( fate == T_ISTMP )
            counts->temp++;
        else if( fate == T_CANTFIND )
            counts->cantfind++;
        else if( fate == T_CANTMAKE && t->actions )
            counts->cantmake++;
        else if( fate >= T_BUILD && fate < T_FATE_BROKEN && t->actions )
            counts->updating++;

        if( !( t->flags & T_FLAG_NOTFILE ) && fate >= T_SPOIL )
            flag = "+";
        else if( t->b1 == T_BIND_EXISTS && p && t->time > p->time )
            flag = "*";
...
make0sort( TAS *chain )
{
        ...
        /* updating each TAS' c->next and c->tail.  Note that we */
